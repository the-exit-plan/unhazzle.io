...
| Item | Description | Source | MVP | Must-Have (ASAP after launch) | Nice-to-Have (revisit later) |
|------|-------------|--------|-----|-----------|--------------|
...
| Editable auto-generated env var names | Users need to customize env var names or rebuild app to match | Jeroen/Eveline | ✓ | | |
| Edit selections on pricing page | Allow editing configuration choices on review/pricing page without going back | Viviana | ✓ | | |
| Multi-container support | Support multiple containers per application (frontend/backend separation) | Jeroen/Eveline | ✓ | | |
| Volume/stateful storage | Ability to attach persistent volumes for stateful applications | Jeroen/Eveline | ✓ | | |
| Static content hosting | Solution for hosting static assets (CDN, object storage) | Matija, Jeroen/Eveline | | ✓ | |
| File uploads/volumes (sames as above?) | Allow uploading files or pulling from GitHub to mount as volumes | Inaki | ✓  | | |
| Private endpoints | Configure private endpoints for third-party services (no internet exposure) | Inaki | | ✓ | |
| Custom service containers | Allow deploying custom services as containers (e.g., Cassandra) | Inaki | | | ✓ |
...
| Item | Description | Source | MVP | Must-Have (ASAP after launch) | Nice-to-Have (revisit later) |
|------|-------------|--------|-----|-----------|--------------|
| NoSQL database support | Add MongoDB, Cassandra, or other NoSQL options | Matija, Inaki | | ✓ | |
